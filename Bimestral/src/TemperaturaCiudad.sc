...
//Ejercicio 2
def variacionTermica(ciudad: Ciudad):Double =
  ciudad.temps.max - ciudad.temps.min
val temperaturas: List[Double] = ciudadesValidas.map(variacionTermica)

//Ejercicio 3
def ciudadMasVarTerm(ciudadesV: List[Ciudad]): Ciudad =
  ciudadesV.maxBy(variacionTermica) // mejoramiento de la funcion usando maxBy
